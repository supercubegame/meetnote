...
- `.github/live-heartbeat.json` 每日定时跑写回的心跳，唯一给仓库外部看的存活证据。
...
- 心跳文件的路径和本文件里写的路径是一组。外部审计被告知去读那个确切路径，路径动了文档没动，
  审计就会读到空气然后报告健康。`test_heartbeat_path_is_documented` 在守。
...
**「cron 还写在 YAML 里」证明不了它还在跑。** 被停用时这个文件一个字都不会变，所以那种检查
对它要防的失效永远为真,和空断言是同一个洞。也没法在仓库内部补：守卫本身得挂在那条
已经停了的定时任务上。

所以每次定时跑最后会把 `.github/live-heartbeat.json` 提交回主干，内容是 UTC 时间戳、
慢闸门退出码与状态、真实响应 id、run 链接。**这个文件只可能由真的跑过的定时任务产生**,
它是正向证据，不是「没有坏消息」。用 `always()` 写，所以接口挂了或漂移了也照样证明 cron 活着；
提交信息带 skip 标记（不带的话心跳落到主干会再触发慢闸门，无限循环，每圈烧一次真实调用）；
推不上去直接让 job 红,一条没送达的心跳和一条死掉的 cron 从外面看一模一样。

外部读法：比 `checked_at` 和今天差多久。超过约 48 小时就说明定时任务没在跑，仓库再绿也一样。
[@闸门审计 Quinn](#-47467068) 每周一读一次。种子文件里 `checked_at` 是 `null`,
意思是「还没有产出过任何一次心跳」，别手改成一个时间，那等于伪造唯一的存活证据。

**注意：写提交信息时别在正文里出现那个 skip 标记。** GitHub 认整条信息，不只认标题,
在说明里引用它会让这次推送完全不跑 CI。这事已经发生过一次。
...
- 心跳能证明定时任务跑过，但**证明不了它明天还会跑**。它是事后信号，不是保证。
...
开新分支把改动重放一遍，开新 PR，关掉旧的。这事已经发生过两次,第二次的症状是「推了代码
但一个 run 都没有」，因为 PR 已经被合掉关闭了，别误判成工作流文件写坏了。

## 人工环节（agent 碰不到）

- 仓库 secrets 里的 `STEPFUN_API_KEY`。
- Actions 的工作流写权限（不开的话报告传不回来，job 会直接红）。
- 定时任务被停用后的重新启用。Quinn 报「心跳过期」时就是这件事，去 Actions 页面点回来。
